...
riscv_emit_shadow_stack_prologue()
{
  ...
  // if zicfiss extension is supported use hardware instructions like sspush and sspopchk
  if (is_zicfiss_p())
  {
...
riscv_emit_shadow_stack_epilogue()
{
  ...
  // redundant variable the symmetry with prologue
  rtx offset = GEN_INT (-UNITS_PER_WORD);
...
/* Disabling CF_RETURN error for the moment for non-zicfiss targets
   for the moment. Software implementation of shadow call stack underway */
